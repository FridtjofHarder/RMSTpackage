...
for (i in 1:51){
  ...
  arm_0 <- create_arm(size = m, accr_time = accrual_time, surv_cure = cure[i], surv_scale = scale_0, 
                      surv_interval = interval_0, loss_scale = loss_to_FU_scale_0, 
                      loss_shape = loss_to_FU_shape_0, follow_time = follow_up)
  arm_1 <- create_arm(size = m, accr_time = accrual_time, surv_cure = cure[i], surv_scale = scale_1, 
...
plot(cure, RMSTR)
model <- lm(RMSTR ~ cure + I(cure^2) + I(cure^3))

myPredict <- predict(model) 
ix <- sort(cure,index.return=T)$ix
lines(cure[ix], myPredict[ix], col=2, lwd=2)  

title("RMSTR against cure fraction for hazard_0 = .4, integration limit = 8, \n exponential model with hazard ratio = 2")
...
title("cure fraction = .5, lambda_0 = .4, lambda_1 = .2")
