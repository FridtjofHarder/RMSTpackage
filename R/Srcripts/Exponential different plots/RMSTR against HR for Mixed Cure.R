...
for (i in 1:41){
  ...
  arm_0 <- create_arm(size = m, accr_time = accrual_time, surv_cure = cure, surv_scale = scale_0, 
                      surv_interval = interval_0, loss_scale = loss_to_FU_scale_0, 
                      loss_shape = loss_to_FU_shape_0, follow_time = follow_up)
  arm_1 <- create_arm(size = m, accr_time = accrual_time, surv_cure = cure, surv_scale = scale_1, 
...
plot(HR, RMSTR)
model <- lm(RMSTR ~ HR + I(HR^2) + I(HR^3))

myPredict <- predict(model) 
ix <- sort(HR,index.return=T)$ix
lines(HR[ix], myPredict[ix], col=2, lwd=2)  

title("RMSTR against HR for hazard_0 = .4, integration limit = 8, \n cure fraction = .3, exponential model")
