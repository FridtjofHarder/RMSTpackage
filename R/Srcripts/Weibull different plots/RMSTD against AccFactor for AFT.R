...
scale_0 <- c(.5) # set scale for exponential dis of survival in control
shape_0 <- c(4)
...
for (i in 1:21){
  ...
  event_times_1 <- rsurv(m, arm_1, include_cured = F)*acc_factor[i]
...
plot(acc_factor, RMSTD)

model <- lm(RMSTD ~ acc_factor + I(acc_factor^2) + I(acc_factor^3))
myPredict <- predict(model) 
ix <- sort(acc_factor,index.return=T)$ix
lines(acc_factor[ix], myPredict[ix], col=2, lwd=2)  
title("RMSTD against acceleraton factor for equal hazards, integration limit = 8, \n cure fraction = .3")
