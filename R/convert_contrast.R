...
#' @param tau time horizon at which to evaluate RMST with \eqn{\mathrm{RMST} = \int_{0}^{\tau}S(t) \,dt}
#'
...
convert_contrast <- function(scale_trt = NULL, shape_trt = 1, scale_ctrl = NULL, shape_ctrl = 1,
                             ...
                             survival_diff = NULL, output = "RMSTD", tau = NULL, extended_output = F){
  # convert from HR
  ...
  if(!is.null(HR)){
    if(!is.null(scale_trt)){
      ...
      print("2nd loop")}
    print(paste("scale_trt is", scale_trt))
    print(paste("shape_trt is", shape_trt))
