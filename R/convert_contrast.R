...
#' @param output A string specifying whether to calculate the difference in RMSTs (\code{"RMSTD"}), the ratio between RMSTs (\code{"RMSTR"}), or the HR (\code{"HR"}).
...
# error management --------------------------------------------------------
  ...
  stopifnot("please specify exactly one scale parameter. Use calculate_contrast() to determine the RMSTD, RMSTR, or HR for fully specified survival functions"
            = xor(is.null(scale_trt), is.null(scale_ctrl)))
