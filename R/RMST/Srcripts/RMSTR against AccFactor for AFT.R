...
scale_0 <- c(.2) # set scale for exponential dis of survival in control
shape_0 <- c(2)
...
for (i in 1:21){
  ...
  arm_0 <- create_arm(size = m, accr_time = accrual_time, surv_scale = scale_0, surve_shape = shape_0, 
                      surv_interval = interval_0, loss_scale = loss_to_FU_scale_0, 
                      loss_shape = loss_to_FU_shape_0, follow_time = follow_up)
  arm_1 <- create_arm(size = m, accr_time = accrual_time, surv_scale = scale_1, surve_shape = shape_1,
