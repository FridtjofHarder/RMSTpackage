...
shape_ratio <- seq(from = 1, to = 5, by = .1)
RMSTR <- rep(NA, 41)
for (i in 1:41){
shape_1 <- shape_0 * shape_ratio[i]
integration_limit <- min(total_length, evaluation_point)
arm_0 <- create_arm(size = m, accr_time = accrual_time, surv_scale = scale_0, surv_shape = shape_0,
                    surv_interval = interval_0, loss_scale = loss_to_FU_scale_0, 
                    loss_shape = loss_to_FU_shape_0, follow_time = follow_up)
arm_1 <- create_arm(size = m, accr_time = accrual_time, surv_scale = scale_1, surv_shape = shape_1,
...
RMSTD <- RMST_splines_1 - RMST_splines_0
i
}
...
plot(shape_ratio, RMSTR)
model <- lm(RMSTR ~ shape_ratio + I(shape_ratio^2) + I(shape_ratio^3))
myPredict <- predict(model) 
ix <- sort(shape_ratio,index.return=T)$ix
lines(shape_ratio[ix], myPredict[ix], col=2, lwd=2)  
title("RMSTR against ratio of shape_0/shape_1 for scale_0 = scale_1 = .4, \n shape_0 = .3, integration limit = 8")
...
title("scale_0 = scale_1 = .4, shape_0 = .3, shape_1 = 1.5")
